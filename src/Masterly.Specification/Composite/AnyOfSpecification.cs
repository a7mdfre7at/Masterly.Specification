...
{
    /// <summary>
    /// Represents a specification where ANY of the given specifications must be satisfied.
    /// </summary>
    /// <typeparam name="T">The type of the object to which the specification is applied.</typeparam>
    public class AnyOfSpecification<T> : Specification<T>
    {
        private readonly IReadOnlyList<ISpecification<T>> _specifications;

        public AnyOfSpecification(params ISpecification<T>[] specifications)
            : this((IEnumerable<ISpecification<T>>)specifications)
        {
        }

        public AnyOfSpecification(IEnumerable<ISpecification<T>> specifications)
        {
        ...
        public override Expression<Func<T, bool>> ToExpression()
        {
            ...
            {
                result = result.Or(_specifications[i].ToExpression());
            }
